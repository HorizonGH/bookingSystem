...
import { serviceService, ServiceDto, UpdateServiceRequest } from '../../../../../../services/service';
import { ApiError } from '../../../../../../services/api';
import MessagePopup from '../../../../../../components/MessagePopup';

export default function EditServicePage() {
  const params = useParams() as { id: string; serviceId: string };
    ...
    const load = async () => {
      try {
        ...
        setFormData({
          ...
          imageUrl: s.imageUrl || '',
          category: s.category || '',
          bufferTimeBefore: s.bufferTimeBefore || 0,
          bufferTimeAfter: s.bufferTimeAfter || 0,
          requiresApproval: s.requiresApproval || false,
          advanceBookingHours: s.advanceBookingHours || 24,
          minBookingHours: s.minBookingHours || 1,
        });
      } catch (e) {
        void e;
        setPopup({ type: 'error', message: 'No se pudo cargar el servicio' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        setPopup({ type: 'error', message: 'Error al actualizar el servicio' });
      }
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-light dark:bg-dark">
        <div className="text-center">
          <p className="text-secondary-500 mb-4">Servicio no encontrado</p>
    ...
    <div className="min-h-screen bg-light dark:bg-dark">
      ...
      <div className="bg-white dark:bg-dark-light border-b border-light-darker dark:border-secondary-700 px-4 py-4">
        <div className="container mx-auto max-w-2xl flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-dark dark:text-light">Editar Servicio</h1>
            <p className="text-xs text-secondary-500 truncate max-w-[200px]">{service.name}</p>
          </div>
        </div>
      </div>
